...
export async function setupCallTransaction(element) {
  element.innerHTML = `test deposit call`;
...
export async function setupSendTransaction(element) {
  element.innerHTML = `test send tx`;
  ...
  const doTx = async () => {
    ...
    const ethSpendAmount = ethers.parseEther("0.0000001");

    // Send transaction with signer
    const tx = await signer.sendTransaction({
      to: "0x857f5c31265f756e89ee64e94D536eFC7d6BEb38",
      value: ethSpendAmount,
      data: ethers.AbiCoder.defaultAbiCoder().encode([{ type: "uint256", name: "value" }], [ethSpendAmount]),
    });
    console.log({ tx })
